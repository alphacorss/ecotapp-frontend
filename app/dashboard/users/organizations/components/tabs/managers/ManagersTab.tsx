...
import { AddEditUser } from '@/app/components/forms/AddEditUser';
import DataTable from '@/app/components/table/Table';
import { UserColumnData } from '@/app/components/table/UserColumn';
import DeleteModalContent from '@/app/components/utils/DeleteModalContent';
import GetItemModal from '@/app/components/utils/GetItemModal';
...
import ViewUser from '@/app/components/view/ViewUser';
...
const ManagersTab = ({ org }: { org: TOrg }) => {
  ...
  const showDetailsModal = (id: string) => {
    orgManagersData.forEach((user: TUser) => {
      if (user._id === id) {
        router.push(`/dashboard/users/organizations?tab=managers&orgId=${org._id}&orgManagerId=${user._id}`);
      }
    });
    orgManager.refetch();
